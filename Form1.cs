...
namespace SinanScrumTable
{
    public partial class Form1 : Form
    {
        ...
        void TaskleriListele2()
        {
            ComboboxItem item = new ComboboxItem();
            komut.CommandText = "SELECT Id, TaskName, TaskCategoryId, TaskStoryId FROM Tasks";
            DataTable tablom = new DataTable();
            OleDbDataAdapter verilericek = new OleDbDataAdapter(komut);
            verilericek.Fill(tablom);
            int rtBoyut = 5;
            ...
            for (int i = 0; i < tablom.Rows.Count; i++)
            {
                ...
                {
                    sayac++;
                    if ((cbStroy.Items[x] as dynamic).Value == tsid) { break; }
                }
                satirY = (80 * sayac);
                ...
                object p = null;
                for (int a = panelgizli.Controls.Count - 1; a >= 0; a--)
                {
                    if (panelgizli.Controls[a].Name == item.Text.Replace(" ", "_"))
                    {
                        p = panelgizli.Controls[a];
                    }
                }
                var pictureBoxx = p as PictureBox;
                ...
                if (tablom.Rows[i][2].ToString() == "2") {
                    panel3.Controls.Add(pictureBoxx);
                    pictureBoxx.Location = TaskYerlestir(2, satirY, pictureBoxx.Name); 
                    objepanel3++; 
                }
                if (tablom.Rows[i][2].ToString() == "3")
                {
                    panel4.Controls.Add(pictureBoxx);
                    pictureBoxx.Location = TaskYerlestir(3, satirY, pictureBoxx.Name);
                    objepanel4++;
                }
                sayac = -1;
                //rtBoyut += 80;
